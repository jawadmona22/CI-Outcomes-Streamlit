...
mode = st.sidebar.radio(
    ...
    ["Cochlear", "Advanced Bionics","IHS"]
)

if mode == "Advanced Bionics":
    set_background("#f0f8ff")  # light blue
    st.subheader("Current mode: AB")
    
    uploaded_file_list = st.file_uploader("Choose your Excel file(s)", type="xlsx",accept_multiple_files=True)
    is_multiple_frequencies = st.checkbox("My file(s) contains multiple frequencies", value=True)
    if not is_multiple_frequencies:
        st.warning("Please ensure your file(s) contain only one frequency. Otherwise, please check the box above and re-upload your files.")
    if len(uploaded_file_list) > 0:
        ...
        for uploaded_file in uploaded_file_list:
            file_index +=1
            if is_multiple_frequencies == False:
                with st.expander(f"File: {uploaded_file.name}"):
                    ...
                    st.write("This file will be treated as a single frequency file.")
            ...
            if is_multiple_frequencies == True:
                with st.expander(f"File: {uploaded_file.name}"):
                            harmonic_data = []
                            # Get indices of time series between 6ms (6000us) and 15ms (15,000 ms)
                            time_window = st.slider(
                            ...
                            for electrode in electrodes: 
                                ...
                                with col2:
                                    ...
                                    for freq in freqs_present:
                                        ...
                                        fundamental_index, threshold = sum_harmonics_by_peak(amplitude, freq_array,freq)
                                        fundamental_indices.append(fundamental_index)
                                    ...
                                    fig_fft.set_size_inches(6, 3)
                                    ax_fft.set_ylabel('Amplitude (microvolts)')
                                    markers = ["r*", "g*", "b*","m*"]  # Add more markers if you have more frequencies
                                    for idx, freq in enumerate(freqs_present):
                                    
                                        ax_fft.plot(freq_array[fundamental_indices[idx]], max_amps[idx], markers[idx], markersize=10, label=f'1st Harmonic: {int(freq)} Hz')
                                    ...
                                    # Append the data for this measurement to the list
                                    multi_harmonic_data.append({
                                        'Recording Electrode':electrode,
                                        f'F0 {freqs_present[0]} (Hz)': max_amps[0] if max_amps[0] > thresholds[0] else 0,
                                        f'F0 {freqs_present[1]} (Hz)': max_amps[1] if max_amps[1] > thresholds[1] else 0,
                                        f'F0 {freqs_present[2]} (Hz)': max_amps[2] if max_amps[2] > thresholds[2] else 0,
                                        f'F0 {freqs_present[3]} (Hz)': max_amps[3] if max_amps[3] > thresholds[3] else 0,
                                    })
                            multi_harmonic_df = pd.DataFrame(multi_harmonic_data)
                                # Apply the style to all values in the DataFrame
                            styled_df = multi_harmonic_df.style.applymap(highlight_zeros)
                            ...
                            for idx, freq in enumerate(freqs_present):
                                    ...
                                    ax.plot(multi_harmonic_df["Recording Electrode"][:], multi_harmonic_df[f'F0 {freqs_present[idx]} (Hz)'][:],'-o')
                                    ax.set_title(f"Amplitude of F0 vs Electrode for {int(freqs_present[idx])} Hz")
                                    ax.set_xlabel("Recording Electrode")
                                    ax.set_ylabel("Amplitude (uV)")
                                    # ax.set_xticklabels(multi_harmonic_df["Recording Electrode"],rotation=45, ha="right")
                                    st.pyplot(fig)
                                    plt.close(fig)
    ...
    set_background("#ffffed")  # light yellow
    ...
    set_background("#F0FAF5")  # light green
    ...
    if uploaded_file_list:
        ...
        for uploaded_file in uploaded_file_list:
            with st.expander(f"File: {uploaded_file.name}", expanded=True):
                ...
                # --- Metadata Extraction ---
                # Read first 24 rows for metadata
                IHS_metadata = pd.read_csv(uploaded_file, sep=",", nrows=24, header=None, index_col=0).dropna(axis=1, how='all').T  
                IHS_metadata.columns = [str(col).strip(':').strip() for col in IHS_metadata.columns]
                
                # Extract valid frequencies (removing NaNs from the CSV structure)
                stim_freq_array = [int(f) for f in IHS_metadata['Stim. Freq.'].values.flatten() if pd.notna(f) and str(f).strip() != '']
                
                # Sampling Period logic (IHS default is 25.0 us)
                smp_period = float(IHS_metadata['Smp. Period'].values.flatten()[0])
                FsRecording = 1000000 / smp_period  # Result: 40000.0
                ms_per_sample = 1000 / FsRecording
                ...
                # --- Analysis Loop (Iterates through ALL detected frequencies) ---
                for idx, freq in enumerate(stim_freq_array):
                    st.markdown(f"### Frequency: {freq} Hz")
                    
                    # Create a new row of columns for every single frequency
                    col1, col2 = st.columns(2)

                    # Dynamic Tag Assignment
                    # IHS files often label duplicates as Column, Column.1, Column.2...
                    suffix = f".{idx}" if idx > 0 else ""
                    b1_tag = f"Buffer1(uV){suffix}"
                    b2_tag = f"Buffer2(uV){suffix}"

                    # Safety check: if the column doesn't exist, try the version without the dot
                    if b1_tag not in df.columns:
                        b1_tag = f"Buffer1(uV):{suffix}" if idx > 0 else "Buffer1(uV)"
                        b2_tag = f"Buffer2(uV):{suffix}" if idx > 0 else "Buffer2(uV)"

                    if b1_tag in df.columns and b2_tag in df.columns:
                        # PHASE SUBTRACTION: (B1 - B2) isolates the phase-following CM
                        voltage = (df[b1_tag] - df[b2_tag]) / 2
                        
                        # Generate Time Axis (ms)
                        x_time = np.arange(len(voltage)) * ms_per_sample
                        
                        with col1: 
                            st.write("**Cochlear Microphonic (Time Domain)**")
                            fig_t, ax_t = plt.subplots(figsize=(6, 3))
                            ax_t.plot(x_time, voltage, linewidth=0.8, color='#2E7D32')
                        ...
                        with col2:
                            ...
                            L = len(voltage)
                            NFFT = 2 ** (int(np.ceil(np.log2(L))) + 2)

                            # Real FFT calculation
                            fft_vals = np.fft.rfft(voltage, n=NFFT) / L
                            amplitude = 2 * np.abs(fft_vals)
                            freq_axis = np.fft.rfftfreq(NFFT, d=1/FsRecording)

                            # Locate peak at stimulus frequency
                            target_f = int(freq)
                            peak_idx = np.argmin(np.abs(freq_axis - target_f))
                            peak_amp = amplitude[peak_idx]

                            fig_f, ax_f = plt.subplots(figsize=(6, 3))
                            # Plot from 10Hz up to 5000Hz
                            ax_f.plot(freq_axis[1:], amplitude[1:], linewidth=0.8, color='#1565C0')
                            ax_f.plot(target_f, peak_amp, 'r*', markersize=10, label=f'Peak at {target_f}Hz')
                        ...
                        st.warning(f"Could not find data columns for {freq}Hz (Index {idx})")
                    
                    st.divider() # Visual break between frequencies
            #                 second_harmonic_amp = 0 ##TODO: Remove placeholder##
